...
bool ComplexBoolExpr::type_safe(SymbolTable& sym_table, std::ostream& out) {
	...
	if (type != type2) {
		if (!(type == BOOL_TRUE && type2 == BOOL_FALSE || type == BOOL_FALSE && type2 == BOOL_TRUE)) {
...
void ComplexBoolExpr::eval(SymbolTable& sym_table, std::ostream& os) {
	...
	if (it_expr_type == INT_TYPE) {
		...
		double val_1 = it_float;
		it_expr_type = -1;
		second_operand->eval(sym_table, os);
		double val_2 = it_float;
		new_bool = comp(val_1, bool_rel, val_2);
	}
	else if (it_expr_type == STRING_TYPE) {
	...
	else if (it_expr_type == BOOL_TRUE || it_expr_type == BOOL_FALSE) {
		bool val_1 = it_bool;
		it_expr_type = -1;
		second_operand->eval(sym_table, os);
		bool val_2 = it_bool;
...
bool comp(bool a, int op, bool b) {
	return a == b;
}
